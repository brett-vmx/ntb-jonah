...
`scripts/gen-chapters.mjs` from three source texts: `source-assets/
32JONNTB.SFM` (Tibetan), `source-assets/Jonah_BSB.rtf` (English, Berean
Standard Bible), and `source-assets/33-JONcmn-cu89s.usfm` (Chinese, CUV —
added when Brett brought Chinese into the app; see "Reading languages vs.
audio tracks" below). Never hand-edit the generated JSON — edit the
generator (or the source files) and re-run `npm run gen-chapters`.

Each chapter is a `blocks` array of `{type:'verse', number, bo, en, cmn}`
and `{type:'image', file}` entries, in reading order. `bo` is an array of
lines (length 1 for prose; multiple lines for chapter 2's poetry, from
`\q1` markers in the SFM) — `en`/`cmn` are plain strings, since neither
source carries per-line poetry breaks. Inline image positions were
hand-verified against `source-assets/NTB Jonah_final copy.pdf` page by
page — see the `INLINE_IMAGES` map in the generator script for the exact
chapter/verse mapping. Don't move images without re-checking the PDF.

### Reading languages vs. audio tracks — related but independent settings
Two different five/three-way concepts, easy to conflate:
- **Reading language** (`TextLang` in settings-store.ts): `'bo' | 'en' |
  'cmn'` — which text is displayed. Tibetan/English/Chinese are three
  genuinely different texts (SFM/BSB/CUV), not the same text relabeled.
- **Dialect** (`Dialect` in settings-store.ts): `'adx' | 'bod' | 'khg' |
  'eng' | 'cmn'` — which audio plays. Really two different kinds of thing
  under one flat type: the three Tibetan dialects are three recordings of
  the *same* Tibetan text (adx/bod/khg — swapping between them never
  changes the displayed text), while `eng`/`cmn` are full audio tracks in
  their own languages. Kept as one flat type/setting anyway, since every
  place that consumes it (the LISTEN-bar popover, `chapter.audio[dialect]`,
...
Text language (Tibetan/English/Chinese), Tibetan font, text size, and
layout (verse/paragraph) are app-wide settings, not controls inside each
chapter modal — all four chosen from **one** header icon (`Settings` — see
"Header icon" below — opening a single bottom sheet), persisted to
`localStorage`, in `src/i18n/settings-store.ts`. Audio dialect briefly
moved entirely out of this sheet into the LISTEN bar's own popover (John's
request), then partially came back: Brett asked for a Tibetan-dialect row
(adx/bod/khg only) right below the Language row, shown only when Tibetan is
selected — see "Audio dialect picker" below for why it lives in **both**
places now. Same localStorage + `CustomEvent` pattern as tenpa's
`language-store.ts`:
...
script/word — `ཡོ་ནཱ།` (Tibetan), "约拿" (Chinese, its usual name in the CUV
text), "Jonah" (English) — and the wordmark-image logo should become plain
"New Tibetan Bible" text whenever the reading language isn't Tibetan (the
Tibetan-script image doesn't read as English or Chinese). `updateHeaderTitle()`
in Layout.astro's script updates both together on `jonah:text-settings-changed`
and on init — setting `#header-title`'s text and `.tibetan`/`.chinese`
class, and toggling which of `#header-logo-img`/`#header-logo-text` is
hidden. There's no Chinese wordmark asset or confirmed Chinese org-name
translation, so Chinese mode deliberately falls back to the same English
text logo as English mode rather than guessing at brand wording — don't
add a guessed Chinese logo translation without confirming with John/Brett
first. **Only active on pages using the default title** — guarded by
