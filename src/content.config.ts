...
// Book introduction (John's request #20) — Tibetan only, one entry
// (src/content/intro/jonah.json), generated by gen-chapters.mjs from
// source-assets/NTB_Jonah_Introduction.rtf. No audio, no per-language
// variants — see "Book introduction" in CLAUDE.md.
...
// NTB Bible introduction (John's request #21) — Tibetan only, one entry
// (src/content/bible-intro/bible-intro.json), generated by gen-chapters.mjs
// from source-assets/Bible introduction for NTB – for NTB PWA apps.rtf.
// Simpler shape than `intro` above: one title (\mt, no \imt pairing — this
// document has no separate book-name/introduction-title split) plus \s/\p
// sections. See "Bible introduction & timeline toggle" in CLAUDE.md.
...
// NTB Creation-to-Christ timeline (John's request #21, alongside the Bible
// introduction above) — Tibetan only, one entry (src/content/timeline/
// timeline.json). No text to parse — each of the 6 pages is a single image
// with its own title baked in, per John — so this is just an ordered list
// of pre-resized webp files (src/assets/timeline/page-{1-6}.webp, see
// "Asset locations" in CLAUDE.md). Generated by gen-chapters.mjs too, for
// the same "don't hand-edit generated JSON" discipline as everything else,
// even though there's no real parsing step here.
